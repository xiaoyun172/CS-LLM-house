...
export interface CustomBackupOptions {
  ...
  shortcutPhrases: boolean; // 添加快捷短语备份选项
  otherData: boolean;
}
...
export async function performCustomBackup(
  ...
  try {
    ...
    // 获取快捷短语数据
    if (options.shortcutPhrases) {
      const shortcutPhrases = await getStorageItem('shortcut_phrases');
      const phraseCategories = await getStorageItem('phrase_categories');

      backupData.shortcutPhrases = {
        phrases: shortcutPhrases && typeof shortcutPhrases === 'string' ? JSON.parse(shortcutPhrases) : [],
        categories: phraseCategories && typeof phraseCategories === 'string' ? JSON.parse(phraseCategories) : []
      };
      console.log('已添加快捷短语数据');
    }
