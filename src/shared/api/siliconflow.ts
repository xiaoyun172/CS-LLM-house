...
export async function generateImage(
  ...
  try {
    ...
    // 处理中文提示词，优化为更适合模型理解的格式
    let optimizedPrompt = params.prompt;

    // 检测是否为中文提示词
    const hasChinese = /[\u4e00-\u9fa5]/.test(params.prompt);
    if (hasChinese) {
      log('INFO', `检测到中文提示词: "${params.prompt}"`);

      // 针对特定的中文绘画指令进行优化
      if (params.prompt.includes('画一只') || params.prompt.includes('画一个')) {
        // 提取关键对象
        const match = params.prompt.match(/画一[只个](.+)/);
        if (match && match[1]) {
          const subject = match[1].trim();
          // 转换为更适合模型理解的英文提示词格式
          optimizedPrompt = `a detailed, high quality image of ${subject}, photorealistic, 4k, detailed`;
          log('INFO', `优化中文提示词: "${params.prompt}" => "${optimizedPrompt}"`);
        }
      } else if (params.prompt.includes('画')) {
        // 处理其他包含"画"的指令
        const match = params.prompt.match(/画(.+)/);
        if (match && match[1]) {
          const subject = match[1].trim();
          optimizedPrompt = `a detailed, high quality image of ${subject}, photorealistic, 4k, detailed`;
          log('INFO', `优化中文提示词: "${params.prompt}" => "${optimizedPrompt}"`);
      ...
      // 保留原始提示词作为日志记录
      log('INFO', `原始提示词: "${params.prompt}", 优化后: "${optimizedPrompt}"`);
    }
    ...
    try {
      ...
      log('INFO', `成功生成 ${imageUrls.length} 张图像，第一张URL: ${imageUrls[0]?.substring(0, 50)}...`);

      // 记录完整的请求-响应对，用于调试
      logApiResponse('SiliconFlow Image Generation', response.status, {
