...
async function getDB(): Promise<IDBPDatabase<AetherLinkDB>> {
  ...
  try {
    ...
    // 创建新连接
    dbPromise = openDB<AetherLinkDB>('aetherlink-db-new', 1, {
      upgrade(db) {
        ...
        if (!db.objectStoreNames.contains('assistants')) {
          const assistantsStore = db.createObjectStore('assistants', { keyPath: 'id' });
        ...
        if (!db.objectStoreNames.contains('imageMetadata')) {
          const imageMetadataStore = db.createObjectStore('imageMetadata', { keyPath: 'id' });
    ...
    try {
      ...
      await indexedDB.deleteDatabase('aetherlink-db-new');
...
export class DataAdapter {
  ...
  async getAssistant(assistantId: string): Promise<Assistant | undefined> {
    try {
      ...
      const db = await getDB();
      const assistant = await db.get('assistants', assistantId);
      return assistant;
    } catch (error) {
  ...
  async saveAssistant(assistant: Assistant): Promise<string> {
    try {
      ...
      const db = await getDB();
      await db.put('assistants', cleanAssistant);
      return assistant.id;
    } catch (error) {
  ...
  async deleteAssistant(assistantId: string): Promise<void> {
    try {
      ...
      const db = await getDB();
      await db.delete('assistants', assistantId);
    } catch (error) {
  ...
  public async cleanupOldDatabases(): Promise<void> {
    try {
      ...
      // 添加所有发现的数据库，除了当前使用的aetherlink-db-new
      for (const db of databases) {
        if (db.name && db.name !== 'aetherlink-db-new' && !oldDbNames.includes(db.name)) {
          oldDbNames.push(db.name);
        }
      }
